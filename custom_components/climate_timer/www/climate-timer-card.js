...
let ClimateTimerCard = class ClimateTimerCard extends i {
    ...
    async _callClimate(service, data = {}) {
    ...
    _renderModes(state) {
        ...
        return b `
      <div class="modes">
        ${this._supportedModes(state).map((mode) => b `
            <button
              class="mode ${active === mode ? "active" : ""}"
              title=${mode}
              @click=${() => this._callClimate("set_hvac_mode", { hvac_mode: mode })}
            >
    ...
    render() {
        ...
        if (!state) {
            return b `<ha-card><div class="warning">Entity not found</div></ha-card>`;
        }
      ...
      <ha-card>
        ...
        <div class="main">
          ...
          <div class="center">
            ...
            ? b `<span class="current">${current.toFixed(1)}</span>`
            : b `<span class="current">${target?.toFixed(0) ?? "--"}</span>`}
...
__decorate([
    n({ attribute: false })
], ClimateTimerCard.prototype, "_config", void 0);
__decorate([
    r()
], ClimateTimerCard.prototype, "_schedule", void 0);
__decorate([
    r()
], ClimateTimerCard.prototype, "_offTime", void 0);
__decorate([
    r()
...
let ClimateTimerCardEditor = class ClimateTimerCardEditor extends i {
    setConfig(config) {
        this._config = config;
    }
    render() {
    ...
    _valueChanged(ev) {
        ...
        const value = "checked" in ev.target
            ? ev.target.checked
            : ev.target.value;
...
__decorate([
    n({ attribute: false })
], ClimateTimerCardEditor.prototype, "hass", void 0);
__decorate([
    r()
], ClimateTimerCardEditor.prototype, "_config", void 0);
ClimateTimerCardEditor = __decorate([
    t("climate-timer-card-editor")
], ClimateTimerCardEditor);

var climateTimerCardEditor = /*#__PURE__*/Object.freeze({
    __proto__: null,
    get ClimateTimerCardEditor () { return ClimateTimerCardEditor; }
});

export { ClimateTimerCard };
